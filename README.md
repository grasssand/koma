...
#### 4. 📚 归档查重
多个路径内递归扫描所有文件夹及压缩包，找出重复文件。

- 支持从文件名提取 `[社团 (作者)] 作品 (系列) ...`等信息，比对查重
- 支持按封面相似度，分析查重
...
#### 方式一：下载可执行文件（推荐普通用户）
前往 [Releases](https://github.com/grasssand/koma/releases) 页面下载最新的 `Koma-Windows.zip`，解压即用。
请确保安装 `ffmpeg.exe` 且编译时开启 `--enable-libsvtav1`，`--enable-libaom`，`--enable-libjxl`，`--enable-libwebp`；或者下载完整内置 `ffmpeg` 的版本 `Koma-Windows-with-FFmpeg.zip`
